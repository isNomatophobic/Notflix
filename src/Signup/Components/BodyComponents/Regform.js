...
function Regform({ parentHistory }) {
  ...
  const Register = async (e) => {
    if (
      ...
      try {
        ...
        console.log(res);
        if (res.status == 200) parentHistory.push("/login");
      } catch (error) {
        setIsLoading((p) => false);
  ...
  return (
    <div className="signupBody sb2">
      ...
      <h3>
        STEP <span className="bold">2</span> OF <span className="bold">2</span>
      </h3>
      <div>
      ...
      <form>
        <div className="registerForm-container">
          <input
            ...
            onFocus={emailFocusIn}
            onBlur={(e) => emailFocusOut(e)}
            style={emailStyle}
          ></input>
          <label
            htmlFor="emailInput"
            className={emailFocused ? "focusedLabel" : ""}
          >
            Email
          </label>
        </div>
        <div className="emailError formError">{emailError}</div>
        <div className="registerForm-container">
          <input
            className="formInput"
            id="passwordInput"
            onChange={(e) => validatePassword(e)}
            onFocus={(e) => passwordFocusIn(e)}
          ...
          <label
            ...
            className={passwordFocused ? "focusedLabel" : ""}
          >
            Add a password
          </label>
        </div>
        <div className="PasswordError formError">{passwordError}</div>
      </form>
      <div
        className={isChecked ? "check-container checked" : "check-container"}
        onClick={Check}
      >
        <input className="formCheck" type="checkbox"></input>
        <label>Please do not email me Netflix special offers.</label>
      </div>
      <Link>
        <button className={isLoading ? "awaitLoad" : ""} onClick={Register}>
          Register
        </button>
      </Link>
    </div>
